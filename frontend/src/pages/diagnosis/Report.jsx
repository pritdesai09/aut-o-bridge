...
export default function Report() {
  ...
  useEffect(() => {
    ...
    api.get(`/diagnosis/report?job_id=${jobId}`).then(r => {
      const data = { ...r.data, category_scores: categories }
      setResult(data)
      ...
      api.post('/diagnosis/save-report', {
        ...
        q_score: qScore,
        alignment_score: r.data.emotion_scores?.alignment_score || 0.5,
        variability_score: r.data.emotion_scores?.variability_score || 0.5,
        final_score: r.data.final?.final_score || 0.5,
        confidence_level: r.data.final?.confidence_level || 'Moderate',
        category_scores: categories
      }).catch(err => console.log('Report save error:', err))

    }).catch(() => {
      const final = {
        confidence_level: 'Moderate',
        percentage: Math.round(qScore * 100),
        message: 'Some indicators observed. A consultation with a specialist is recommended.',
        final_score: qScore
      }
      const data = {
        final,
  ...
  const pct = final.percentage || Math.round((final.final_score || 0.5) * 100)
          ...
          {[
            ...
            <div key={label} className="mb-4">
              ...
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div className={`${color} h-2 rounded-full`} style={{width:`${(val||0)*100}%`}} />
              </div>
            </div>
          ))}
        </div>
